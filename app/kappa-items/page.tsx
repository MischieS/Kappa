import KappaItemsClient from "./KappaItemsClient";
...
export default async function KappaItemsPage() {
  ...
  try {
    ...
    console.error("Failed to load Kappa items ref on server", error);
  }

  try {
    initialUserProgress = await getInitialUserProgress();
  } catch (error) {
    console.error("Failed to load Kappa items user progress on server", error);
  }

  return <KappaItemsClient initialTasks={initialTasks} initialUserProgress={initialUserProgress} />;
}
