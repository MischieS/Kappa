...
interface KappaItemsClientProps {
...
const KAPPA_ITEMS_CACHE_KEY = "tasksRefCache-v1";
...
export default function KappaItemsClient({ initialTasks, initialUserProgress }: KappaItemsClientProps) {
    ...
    async function loadData() {
      ...
      try {
        ...
        if (typeof window !== "undefined") {
          try {
            const rawCache = window.localStorage.getItem(KAPPA_ITEMS_CACHE_KEY);
            if (rawCache) {
        ...
        if (!rawTasks) {
          const tasksRes = await fetch("/api/v1/ref/tasks", {
            method: "GET",
          });
          ...
          if (!tasksRes.ok || !tasksJson || !Array.isArray(tasksJson.tasks)) {
            console.error("tarkov.dev tasks error (kappa items)", { status: tasksRes.status, tasksJson });
            setError("Failed to load Kappa items from tarkov.dev");
            setLoading(false);
            return;
          }
        ...
        const collectorTasks: TaskForItemsRef[] = rawTasks.filter((task) => {
          const title = String(task?.name ?? "");
          const normalized = title.trim().toLowerCase();
          return normalized === "the collector" || normalized === "collector";
        });
        ...
        const mappedQuests: QuestForItems[] = collectorTasks.map((task) => {
        ...
        if (!cancelled) {
          setTasksForItems(collectorTasks);
        ...
        console.error("Failed to load Kappa items", err);
        if (!cancelled) {
          setError("Failed to load Kappa items");
        }
      } finally {
        if (!cancelled) {
          setLoading(false);
  ...
  useEffect(() => {
    ...
    try {
      const cookie = document.cookie
        .split("; ")
        .find((entry) => entry.startsWith("kappaItemsSettings="));
      if (!cookie) return;
  ...
  useEffect(() => {
    ...
    try {
      ...
      document.cookie = `kappaItemsSettings=${encodeURIComponent(payload)}; path=/; expires=${expires.toUTCString()}`;
    } catch {
  ...
  useEffect(() => {
    ...
    try {
      ...
      window.localStorage.setItem("kappaItemsSummary", payload);
    } catch {
      // ignore localStorage errors
    }
  }, [overallStats]);
    ...
    try {
      if (canSyncToServer) {
        try {
          ...
          console.error("Failed to update Kappa item progress", err);
        }
      }
    } catch {
  ...
  return (
    <AppShell
      title="Kappa Items"
      subtitle="Track Collector (Kappa) quest item requirements with the same tools as quest items."
    >
      <section className="space-y-4">
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div className="flex-1">
            <div className="flex items-center gap-2 rounded-lg border border-zinc-800/80 bg-zinc-950/80 px-3 py-1.5 text-sm text-zinc-100 focus-within:border-emerald-500/60 focus-within:ring-1 focus-within:ring-emerald-500/40">
              ...
              <input
                placeholder="Search Kappa items"
          ...
          <div className="flex flex-wrap items-center gap-2 text-xs">
            <div className="inline-flex rounded-full bg-zinc-900/80 p-0.5 text-[11px]">
              <button
                ...
                className={`rounded-full px-3 py-1 ${itemsTab === "needed" ? "bg-emerald-600 text-emerald-50" : "text-zinc-300"}`}
              >
                Needed
              </button>
              <button
                ...
                className={`rounded-full px-3 py-1 ${itemsTab === "found" ? "bg-emerald-600 text-emerald-50" : "text-zinc-300"}`}
              >
                Found
              </button>
            </div>
        ...
        {loading && aggregatedItems.length === 0 ? (
          <p className="text-xs text-zinc-400">Loading Kappa items…</p>
        ) : null}

        {!loading && filteredItems.length === 0 ? (
          <p className="text-xs text-zinc-400">No Kappa items match your filters yet.</p>
        ) : null}
                  ...
                  <CardHeader className="flex flex-col gap-1 pb-2">
                    <div className="flex items-center gap-2">
                      ...
                      <div className="min-w-0">
                        ...
                        {viewMode === "items-per-quest" && primaryQuest && teamProgress && teamProgress.members.length > 0 ? (
